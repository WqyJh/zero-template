...
// Prefix presents the prefix to swagger resources (default /swagger).
func Prefix(prefix string) func(*Config) {
	return func(c *Config) {
...
func Handler(configFns ...func(*Config)) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		path, ok := strings.CutPrefix(r.RequestURI, config.Prefix)
		if !ok {
		...
		case "":
			http.Redirect(w, r, config.Prefix+"/index.html", http.StatusMovedPermanently)
		default:
